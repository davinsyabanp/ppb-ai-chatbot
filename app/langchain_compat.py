...
def create_stuff_documents_chain(
    ...
    """
    try:
        # Try newer LangChain approach (1.0+) via langchain package
        from langchain.chains.combine_documents import create_stuff_documents_chain as _create_stuff
        print(f"[LANGCHAIN_COMPAT] Using native create_stuff_documents_chain")
        return _create_stuff(llm=llm, prompt=prompt, document_variable_name=document_variable_name)
    except (ImportError, AttributeError) as e:
        print(f"[LANGCHAIN_COMPAT] Native create_stuff failed: {e}, using manual chain...")
        
        # Fallback: Manual chain that ACTUALLY WORKS
        ...
        def format_and_invoke(input_data):
            ...
            print(f"[LANGCHAIN_COMPAT] format_and_invoke: docs={len(docs) if isinstance(docs, list) else '?'}, formatted={len(formatted_docs)} chars")
